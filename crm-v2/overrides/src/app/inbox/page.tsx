...
import { Bot, Inbox, Loader2, Mail, RefreshCw, Search, Send, UserRound } from "lucide-react";
...
export default function InboxPage() {
  ...
  async function loadThreads(preferredId?: string | null) {
    setLoading(true);
    try {
      const response = await fetch(`/api/inbox?filter=${filter}&search=${encodeURIComponent(search)}`, { cache: "no-store" });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Не удалось загрузить почту");
      const list = (payload.threads || []) as EmailThread[];
      setThreads(list);
      const wanted = preferredId || selectedId;
      const nextId = wanted && list.some((item) => item.id === wanted) ? wanted : list[0]?.id || null;
  ...
  useEffect(() => {
    ...
    // Filter/search intentionally trigger a fresh server query.
  ...
  async function sync() {
    ...
    try {
      ...
      if (!payload.configured) toast.error("Сначала подключи почту в Настройках");
      else toast.success(payload.imported ? `Добавлено писем: ${payload.imported}` : "Почта синхронизирована");
  ...
  async function classify(isService: boolean) {
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Запросы клиентов — как чат. Автоматические уведомления отделены в «Сервисные».
          </p>
        </div>
      ...
      <div className="grid min-h-[68vh] grid-cols-1 overflow-hidden rounded-xl border bg-background xl:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="border-b xl:border-b-0 xl:border-r">
          <div className="space-y-3 border-b p-3">
            <div className="relative">
              ...
              <Input className="pl-9" placeholder="Имя, email или тема..." value={search} onChange={(event) => setSearch(event.target.value)} />
            </div>
            <div className="flex gap-1 rounded-lg bg-muted p-1">
              {filters.map((item) => (
                <button
                  ...
                  className={`flex-1 rounded-md px-2 py-1.5 text-xs font-medium ${filter === item.value ? "bg-background shadow-sm" : "text-muted-foreground"}`}
                >
                  {item.label}
                </button>
              ))}
          ...
          <div className="max-h-[62vh] overflow-y-auto xl:max-h-[calc(100vh-280px)]">
            {loading && !threads.length ? (
              <div className="flex justify-center p-8 text-sm text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Загрузка...</div>
            ) : !threads.length ? (
              <div className="p-8 text-center text-sm text-muted-foreground"><Mail className="mx-auto mb-2 h-8 w-8 opacity-40" />Писем пока нет.</div>
            ) : threads.map((thread) => (
              <button
                key={thread.id}
                type="button"
                onClick={() => setSelectedId(thread.id)}
                className={`w-full border-b p-4 text-left hover:bg-muted/50 ${selectedId === thread.id ? "bg-muted" : ""}`}
              >
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="truncate text-sm font-semibold">{thread.remoteName || thread.remoteEmail}</span>
                      {thread.unreadCount > 0 && <Badge className="ml-auto h-5 px-1.5 text-[10px]">{thread.unreadCount}</Badge>}
                    </div>
                    <div className="truncate text-xs text-muted-foreground">{thread.subject}</div>
                    <div className="mt-1 line-clamp-2 text-xs text-muted-foreground">
                      {thread.lastDirection === "outgoing" ? "Вы: " : ""}{thread.lastSnippet || "—"}
                    </div>
                    <div className="mt-2 text-[10px] text-muted-foreground">{dateLabel(thread.lastMessageAt)}</div>
                  </div>
                </div>
              </button>
            ))}
        ...
        <section className="flex min-h-[520px] flex-col">
          {!selectedId ? (
            <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">Выбери диалог слева</div>
              ...
              <header className="flex flex-col gap-3 border-b p-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant={detail.thread.isService ? "secondary" : "default"}>{detail.thread.isService ? "Сервисное" : "Клиент"}</Badge>
                  </div>
                  <div className="truncate text-sm text-muted-foreground">{detail.thread.remoteEmail}</div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => classify(!detail.thread.isService)}>
                ...
                {detail.messages.map((message) => {
                  ...
                  return (
                    <div key={message.id} className={`flex ${outgoing ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[88%] rounded-2xl border px-4 py-3 shadow-sm sm:max-w-[75%] ${outgoing ? "border-blue-600 bg-blue-600 text-white" : "bg-background"}`}>
                        <div className={`mb-2 flex gap-3 text-[10px] ${outgoing ? "text-blue-100" : "text-muted-foreground"}`}>
                          <span>{outgoing ? "Вы" : message.fromName || message.fromEmail}</span><span>{dateLabel(message.receivedAt)}</span>
                        </div>
              ...
              <footer className="border-t p-3">
                <div className="flex gap-2">
                  <textarea
                    ...
                    className="min-h-[76px] flex-1 resize-y rounded-md border bg-background px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  />
                  <Button className="h-auto px-4" onClick={send} disabled={sending || !draft.trim()}>
                    {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                  </Button>
                </div>
                <div className="mt-1 text-[10px] text-muted-foreground">⌘/Ctrl + Enter — отправить</div>
              </footer>
            </>
          ) : null}
        </section>
