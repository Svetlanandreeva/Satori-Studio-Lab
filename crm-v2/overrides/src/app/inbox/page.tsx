      ...
      <div className="grid min-h-[68vh] grid-cols-1 overflow-hidden rounded-xl border bg-background xl:grid-cols-[320px_minmax(0,1fr)]">
        ...
        <section className="flex min-h-[520px] flex-col">{!selectedKey ? <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">Выбери диалог слева</div> : detailLoading && !detail ? <div className="flex flex-1 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin" /></div> : detail ? <>
          ...
          <div className="flex-1 overflow-y-auto bg-slate-50/60 p-4 xl:max-h-[calc(100vh-410px)]">
            {detail.documents.length>0 && <div className="mb-4 rounded-xl border bg-white p-3"><div className="mb-2 flex items-center gap-2 text-xs font-semibold"><Paperclip className="h-3.5 w-3.5"/>Вложения в переписке <Badge variant="secondary">{detail.documents.length}</Badge></div><div className="flex flex-wrap gap-2">{detail.documents.map(doc=><a key={doc.id} href={`/api/contacts/${detail.contactId}/documents/${doc.id}`} target="_blank" rel="noreferrer" className="flex max-w-[280px] items-center gap-2 rounded-lg border bg-slate-50 px-3 py-2 text-xs hover:bg-white"><FileText className="h-4 w-4 shrink-0"/><span className="truncate">{doc.name}</span><span className="shrink-0 text-muted-foreground">{Math.max(1,Math.round(doc.sizeBytes/1024))} КБ</span></a>)}</div><div className="mt-2 text-[10px] text-muted-foreground">Вложения сохранены в карточке клиента и доступны AI-менеджеру для анализа.</div></div>}
            <div className="space-y-2">{detail.messages.map((message) => { const outgoing = message.direction === "outgoing"; return <div key={message.id} className={`flex ${outgoing ? "justify-end" : "justify-start"}`}><div className={`max-w-[82%] rounded-2xl px-4 py-2.5 sm:max-w-[72%] ${outgoing ? "bg-slate-900 text-white" : "border bg-white"}`}><div className={`mb-2 flex gap-3 text-[10px] ${outgoing ? "text-slate-300" : "text-muted-foreground"}`}><span>{outgoing ? "Вы" : message.sender || detail.title}</span><span>{dateLabel(message.receivedAt)}</span></div><div className="whitespace-pre-wrap break-words text-sm leading-relaxed">{message.bodyText || "(пустое сообщение)"}</div></div></div>; })}</div></div>
