  ...
  async function loadDetail(key: string) {
    ...
    try {
      ...
      if (channel === "telegram") {
        ...
        setDetail({ channel: "email", threadId: id, contactId: payload.contact?.id || payload.thread?.contactId || null, title: payload.contact?.name || payload.thread?.remoteName || payload.thread?.remoteEmail || "Почта", subtitle: [payload.thread?.remoteEmail, payload.thread?.subject].filter(Boolean).join(" · "), isService: Boolean(payload.thread?.isService), pipelineStage: payload.deal?.stageName || null, documents: payload.documents || [], messages: (payload.messages || []).map((message: any) => ({ id: message.id, direction: message.direction, bodyText: message.bodyText || "", receivedAt: message.receivedAt, sender: message.direction === "outgoing" ? "Вы" : message.fromName || message.fromEmail, sourceMessageId: message.sourceMessageId || null })) });
      }
  ...
  return (
    <div className="flex h-[calc(100vh-8.5rem)] min-h-[620px] flex-col gap-3 overflow-hidden">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden rounded-xl border bg-background md:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <section className="flex min-h-0 min-w-0 flex-col overflow-hidden">{!selectedKey ? <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">Выбери диалог слева</div> : detailLoading && !detail ? <div className="flex flex-1 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin" /></div> : detail ? <>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto bg-slate-50/60 p-4">
            ...
            <div className="space-y-2">{detail.messages.map((message) => { const outgoing = message.direction === "outgoing"; const docs=detail.documents.filter(doc=>doc.sourceMessageId && message.sourceMessageId && doc.sourceMessageId===message.sourceMessageId); return <div key={message.id} className={`flex ${outgoing ? "justify-end" : "justify-start"}`}><div className={`max-w-[82%] rounded-2xl px-4 py-2.5 sm:max-w-[72%] ${outgoing ? "bg-slate-900 text-white" : "border bg-white"}`}><div className={`mb-2 flex gap-3 text-[10px] ${outgoing ? "text-slate-300" : "text-muted-foreground"}`}><span>{outgoing ? "Вы" : message.sender || detail.title}</span><span>{dateLabel(message.receivedAt)}</span></div><div className="whitespace-pre-wrap break-words text-sm leading-relaxed">{message.bodyText || (docs.length?"":"(пустое сообщение)")}</div>{docs.length>0&&<div className="mt-2 space-y-1.5">{docs.map(doc=><button type="button" key={doc.id} onClick={()=>setPreviewDoc(doc)} className={`flex w-full items-center gap-2 rounded-lg border px-2.5 py-2 text-left text-xs ${outgoing?"border-slate-700 bg-slate-800":"bg-slate-50"}`}><FileText className="h-4 w-4 shrink-0"/><span className="min-w-0 flex-1 truncate">{doc.name}</span><span className="shrink-0 opacity-60">{Math.max(1,Math.round(doc.sizeBytes/1024))} КБ</span><Badge variant="secondary" className="text-[9px]">AI</Badge></button>)}</div>}</div></div>; })}</div>{detail.documents.some(doc=>!detail.messages.some(m=>m.sourceMessageId&&m.sourceMessageId===doc.sourceMessageId))&&<div className="mt-4 rounded-xl border border-dashed bg-white p-3"><div className="mb-2 text-xs font-semibold">Вложения из старой переписки</div><div className="flex flex-wrap gap-2">{detail.documents.filter(doc=>!detail.messages.some(m=>m.sourceMessageId&&m.sourceMessageId===doc.sourceMessageId)).map(doc=><button type="button" key={doc.id} onClick={()=>setPreviewDoc(doc)} className="flex items-center gap-2 rounded-lg border px-3 py-2 text-left text-xs"><Paperclip className="h-3.5 w-3.5"/><span>{doc.name}</span></button>)}</div></div>}</div>
