...
function subjectValue(value: unknown): string {
  return String(value || "Без темы").trim() || "Без темы";
}
...
function looksServiceMail(mail: ParsedMail, remoteEmail: string, config: EmailIntegrationConfig): boolean {
  ...
  if (/^(no[._-]?reply|do[._-]?not[._-]?reply|notifications?|mailer-daemon|postmaster|bounce|robot|bot)([+._-]|$)/i.test(localPart)) {
    return true;
  }
...
function findContactByEmail(email: string) {
  ...
  return db
    ...
    .from(contacts)
    .all()
    .find((contact) => String(contact.email || "").trim().toLowerCase() === normalized) || null;
}

function ensureContact(remoteEmail: string, remoteName: string, isService: boolean) {
  const existing = findContactByEmail(remoteEmail);
  if (existing) {
    if (!isService && (existing.temperature === "cold" || existing.qualification === "new")) {
      db.update(contacts)
        .set({
          ...
          score: Math.max(40, Number(existing.score || 0)),
          updatedAt: new Date(),
        })
  ...
  return db
    ...
    .values({
      ...
      name: remoteName || remoteEmail.split("@")[0] || "Email контакт",
      email: remoteEmail,
      phone: null,
      company: null,
      source: "email",
      ...
      score: 40,
      notes: "Создан автоматически из входящего письма",
      createdAt: now,
      updatedAt: now,
    })
    .returning()
...
function findOrCreateThread(input: {
  ...
  // Deliberately group by correspondent rather than strict RFC thread. In the
  // CRM the desired UX is one continuous chat with the same person/company,
  // even when they start a fresh subject later.
  ...
  if (!thread) {
    thread = db
      ...
      .values({
        ...
        lastSnippet: snippet(input.bodyText),
...
function upsertParsedMessage(input: {
  ...
  const serviceByHeaders = looksServiceMail(input.mail, remote.address, input.config);
  // An already known CRM contact is treated as a real conversation unless the
  // sender was explicitly put into the ignore list.
  const explicitlyIgnored = senderExplicitlyIgnored(remote.address, input.config.ignoreSenders);
  const isService = explicitlyIgnored || (!knownContact && serviceByHeaders);
  const contact = ensureContact(remote.address, remote.name, isService);
  const receivedAt = input.mail.date || input.internalDate || new Date();
  const thread = findOrCreateThread({
    remoteEmail: remote.address,
  ...
  db.insert(emailMessages)
    .values({
      ...
      references: input.mail.references
        ? Array.isArray(input.mail.references)
          ? input.mail.references.join(" ")
          : String(input.mail.references)
        : null,
      direction,
      folder: input.folder,
      remoteUid: input.uid,
      fromEmail: fromEmail || account,
      fromName: from?.name || null,
      toEmail: to.map((item) => item.address).join(", ") || account,
      subject,
      bodyText,
      isService,
      isRead: direction === "outgoing" || !input.unread,
      receivedAt,
      createdAt: now,
    })
    .run();
    ...
    for (let offset = 0; offset < capped.length; offset += batchSize) {
      ...
      for await (const message of client.fetch(
        ...
        try {
          ...
          console.error("Email message parse failed", path, message.uid, error);
        }
      }
    }
  } finally {
  ...
  activeSync = (async () => {
    ...
    try {
      ...
      for (const folder of folders) {
        imported += await syncMailbox(client, folder, config, since);
      }
      ...
      const message = error instanceof Error ? error.message : "Ошибка синхронизации почты";
      setSetting(INTEGRATION_KEYS.emailLastSyncError, message.slice(0, 500));
      throw error;
    } finally {
...
export async function testEmailConnection(): Promise<{ imap: boolean; smtp: boolean }> {
  ...
  const transporter = createSmtp(config);
...
export function listEmailThreads(filter: "client" | "service" | "all" = "client", search = "") {
  ...
  return db
    ...
    .filter((thread) => {
      ...
      return [thread.remoteName, thread.remoteEmail, thread.subject, thread.lastSnippet]
        .filter(Boolean)
...
export function getEmailThread(threadId: string) {
  ...
  const contact = thread.contactId
    ? db.select().from(contacts).where(eq(contacts.id, thread.contactId)).get() || null
    : null;
...
export function setThreadService(threadId: string, isService: boolean) {
  ...
  if (!isService && !contactId) {
    const contact = ensureContact(thread.remoteEmail, thread.remoteName || "", false);
    contactId = contact?.id || null;
  }
  db.update(emailThreads)
    .set({ isService, contactId, updatedAt: new Date() })
    .where(eq(emailThreads.id, threadId))
    .run();
  return db.select().from(emailThreads).where(eq(emailThreads.id, threadId)).get();
}

export async function replyToEmailThread(threadId: string, body: string) {
