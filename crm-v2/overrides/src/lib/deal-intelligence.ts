...
export function dealConversationContext(dealId:string){
 ...
 // Never write an amount merely because a currency-looking number appeared in conversation.\n // It must be explicitly labelled and corroborated by a second occurrence or readable client document.\n if(detectedAmount>0 && (Number(deal.value||0)<=0||oldAutoSuspect)) sqlite.prepare("UPDATE deals SET value=?,updated_at=? WHERE id=?").run(detectedAmount,Date.now(),dealId);\n else if(oldAutoSuspect&&!detectedAmount) sqlite.prepare("UPDATE deals SET value=0,updated_at=? WHERE id=?").run(Date.now(),dealId);\n // Previous buggy versions wrote 1–2M RUB artifacts. Do not allow those stale values to become evidence for later AI passes.\n if(oldAutoSuspect) deal.value=detectedAmount||0;
