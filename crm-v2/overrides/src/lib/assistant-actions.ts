...
export function prepareDocumentDelivery(contactId:string,documentId:string){
 ...
 const contact=sqlite.prepare("SELECT id,name,email,telegram_chat_id AS telegramChatId FROM contacts WHERE id=?").get(contactId) as any;
 if(!contact) throw new Error("Клиент не найден");
 return {requiresConfirmation:true,contact:{id:contact.id,name:contact.name,email:contact.email,telegramChatId:contact.telegramChatId},document:{id:doc.id,name:doc.name,kind:doc.kind,sizeBytes:doc.sizeBytes,previewUrl:`/api/contacts/${contactId}/documents/${documentId}`},actions:["send_document"]};
}
...
export async function confirmDocumentDelivery(input:{contactId:string;documentId:string;channel:"telegram"|"email";threadId?:string;caption?:string}){
 ...
 if(input.channel==="telegram"){
   const contact=sqlite.prepare("SELECT telegram_chat_id AS chatId FROM contacts WHERE id=?").get(input.contactId) as any;
   if(!contact?.chatId) throw new Error("У клиента нет Telegram-чата");
   const sent=await sendTelegramDocument({chatId:String(contact.chatId),filename:doc.name,bytes,mimeType:doc.mimeType,caption:input.caption||null});
 ...
 sqlite.prepare("INSERT INTO activities(id,contact_id,type,content,created_at) VALUES(?,?,?,?,?)").run(crypto.randomUUID(),input.contactId,"ai_manager",`Документ «${doc.name}» отправлен клиенту через ${input.channel}`,Date.now());
 return {sent:true};
}
...
export function confirmPayment(dealId:string,amount:number,evidence:string){
 ...
 if(deal.contactId) sqlite.prepare("INSERT INTO activities(id,contact_id,type,content,created_at) VALUES(?,?,?,?,?)").run(crypto.randomUUID(),deal.contactId,"ai_manager",`Оплата подтверждена: ${(paid/100).toLocaleString("ru-RU")} ₽. Основание: ${String(evidence||"подтверждено пользователем").slice(0,400)}`,now);
