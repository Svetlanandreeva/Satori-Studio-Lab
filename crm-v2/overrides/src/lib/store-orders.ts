...
export function ensureStorePipelineStage(): void {
  const existing = db
    ...
    .from(pipelineStages)
    .all()
    .find((stage) => stage.name === STORE_STAGE_NAME);
  if (existing) return;

  // Keep the existing generic "Доставка" stage, then add the explicit point at
  // which the parcel has actually been handed off to the client/carrier.
...
function stageForOrder(order: StoreOrder): string | null {
  ...
  if (payment === "canceled" || fulfillment === "Отменён") return "Отказ";
...
export function syncStoreOrder(order: StoreOrder) {
  ...
  // Do not create a new CRM deal just because someone opened checkout and
  // cancelled payment. Existing migrated cancelled attempts are still found
  // above and moved to "Отказ" so historical data becomes truthful.
  if (!existingDeal && String(order.status || "").toLowerCase() !== "paid") {
    return { orderId, skipped: true, reason: "not-paid" };
  }

  let contact = findContact(order, existingDeal);
  if (!contact && String(order.status || "").toLowerCase() === "paid") {
    const now = new Date();
    contact = db
      .insert(contacts)
      .values({
        ...
        score: 100,
        notes: `[store-customer-order:${orderId}]`,
        createdAt: now,
        updatedAt: now,
      })
      .returning()
  ...
  if (!contact) {
    return { orderId, skipped: true, reason: "contact-not-found" };
  }

  const isPaid = String(order.status || "").toLowerCase() === "paid";
  if (isPaid) {
    // A person who has actually paid is not a cold lead anymore. Purchase is
    // the strongest qualification signal available to the CRM.
    db.update(contacts)
      .set({
        temperature: "hot",
        qualification: "qualified",
        score: Math.max(100, Number(contact.score || 0)),
        updatedAt: new Date(),
      })
  ...
  if (!targetStageName) {
    return { orderId, contactId: contact.id, skipped: true, reason: "payment-pending" };
  }
  ...
  if (!existingDeal) {
    const created = db
      ...
      .values({
        ...
        expectedClose: null,
        probability: probabilityForStage(targetStageName),
  ...
  const isCancelled = targetStageName === "Отказ";
  if (isCancelled) {
    nextStage = targetStage;
  } else if (!currentStage || currentStage.isLost || targetStage.isWon || targetStage.order > currentStage.order) {
    // Paid order progress may advance automatically, but reconciliation never
    // drags a manually advanced order backwards.
    nextStage = targetStage;
  }

  db.update(deals)
    .set({
      title: existingDeal.title || titleForOrder(order),
      value,
      stageId: nextStage.id,
      probability: probabilityForStage(nextStage.name),
...
const CRM_TO_STOREFRONT_STATUS: Record<string, string> = {
  ...
  "Готово": "Собран",
  [STORE_STAGE_NAME]: "Отправлен клиенту",
  "Завершено": "Выполнен",
};
